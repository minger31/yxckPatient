...
public class CircleDot  extends View{
	...
	public CircleDot(Context context,LinearLayout lt, int courseCount,int current) {
	 ...
	 private void addDot(int current){
		 ...
		 for (int i = 0; i < count; i++) {
		     ...
		     if (i == current) {
				dot.setImageResource(R.drawable.bg_indicat_select);
			}else{
				dot.setImageResource(R.drawable.bg_indicat_no);
			}
	...
	public void selected(int selectPosition){
		for (int i = 0; i < count; i++) {
			if (selectPosition == i) {
				dotList.get(i).setImageResource(R.drawable.bg_indicat_select);
			}else{
				dotList.get(i).setImageResource(R.drawable.bg_indicat_no);
			}
		}
	}
}
