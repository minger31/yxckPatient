...
import com.patient.library.http.multipart.MultipartEntity;
import com.patient.library.http.multipart.content.ContentBody;
import com.patient.library.http.multipart.content.FileBody;
import com.patient.library.http.multipart.content.InputStreamBody;
import com.patient.library.http.multipart.content.StringBody;
...
public class RequestParams {
    ...
    public void addQueryStringParameter(String name, String value) {
        if (queryStringParams == null) {
            ...
            queryStringParams.add(new BasicNameValuePair("json","Y"));
        }
    ...
    public void printAllParams(){
    	if (!TextUtils.isEmpty(LoginPreference.getUserInfo().partyId)) {
    		addHeader("", "");
    		this.headers.add(new BasicHeader("USERNAME", LoginPreference.getUserInfo().partyId));
    		this.headers.add(new BasicHeader("PASSWORD", LoginPreference.getUserInfo().password));
		}
        
    	if(headers!=null&&headers.size()>0){
    		for(Header item:headers){
    			LogUtil.d2File("header parameter :"+item.getName()+" "+item.getValue());
    		}  		
    	}else{
    		LogUtil.d2File("headers params is empty");
    	}
    	
    	if(queryStringParams!=null&&queryStringParams.size()>0){
    		for(NameValuePair item:queryStringParams){
    			LogUtil.d2File("queryStringParams parameter :"+item.getName()+" "+item.getValue());
    		}  		
    	}else{
    		LogUtil.d2File("queryStringParams params is empty");
    	}
    	
    	if(bodyParams!=null&&bodyParams.size()>0){
    		for(NameValuePair item:bodyParams){
    			LogUtil.d2File("bodyParams parameter :"+item.getName()+" "+item.getValue());
    		}  		
    	}else{
    		LogUtil.d2File("bodyParams params is empty");
    	}
    	if(fileParams!=null&&fileParams.size()>0){
	    	for (ConcurrentHashMap.Entry<String, ContentBody> entry : fileParams.entrySet()) {
	    		LogUtil.d2File("fileParams parameter :"+entry.getKey()+" "+entry.getValue());
	        }
    	}else{
    		LogUtil.d2File("fileParams params is empty");
    	}
    }
