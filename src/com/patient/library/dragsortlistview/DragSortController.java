...
import com.patient.library.dragsortlistview.DragSortListView.ItemRemoveCallBack;
...
public class DragSortController extends SimpleFloatViewManager implements
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (e1 == null || e2 == null) {
			// 有时可能为空
			LogUtil.d("MotionEvent e1 == null or e2 == null");
			return false;
		}
		...
		int speedScale = 0;
		if (distanceY != 0) {
			speedScale = (int) Math.abs(distanceX / distanceY);
		} else {
			speedScale = 2;
		}
			...
			if (mHitPos != MISS) {
				...
				if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop
					...
					startDrag(mHitPos, deltaX, deltaY);
						...
						&& Math.abs(x2 - x1) > mTouchSlop && speedScale >= 2
						&& mRemoveEnabled) {
					mIsRemoving = true;
					startDrag(mFlingHitPos, deltaX, deltaY);
				}
			} else if (mFlingHitPos != MISS) {
				if (Math.abs(x2 - x1) > mTouchSlop && speedScale >= 2
						&& mRemoveEnabled) {
					mIsRemoving = true;
					startDrag(mFlingHitPos, deltaX, deltaY);
	...
	private GestureDetector.OnGestureListener mFlingRemoveListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public final boolean onFling(MotionEvent e1, MotionEvent e2,
			...
			int speedScale = 0;
			if (velocityY != 0) {
				speedScale = (int) Math.abs(velocityX / velocityY);
			} else {
				speedScale = 2;
			}
			if (mRemoveEnabled && mIsRemoving && speedScale >= 2) {
