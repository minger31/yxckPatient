...
public class SideBar extends View {
            ...
            if (oldChoose != c) {
                if (c >= 0 && c < length) {
                    if (popView != null) {
                        ...
                        if (!isShown) {
                            LogUtil.d(TAG, "ACTION_DOWN addView");
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (oldChoose != c) {
                if (c >= 0 && c < length) {
                    if (popView != null) {
                        if (!isShown) {
                            LogUtil.d(TAG, "ACTION_MOVE addView");
                            winManager.addView(popView, layoutParam);
                            isShown = true;
                        }
                    }
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            LogUtil.d(TAG, "handler handleMessage");
            if (popView != null) {
                if (isShown) {
                    LogUtil.d(TAG, "handler removeView");
